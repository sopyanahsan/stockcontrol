...
export const RECEIVING_INCLUDE = {
  ...
  supplierRel: { select: { id: true, code: true, name: true, isActive: true } },
  lines: {
    include: {
  ...
  // Outstanding receipt activity — belongs to the same GRN.
  outstandingActivities: {
    include: { item: { select: { sku: true, name: true } }, operator: { select: { name: true } } },
    orderBy: { createdAt: 'desc' },
  },
}

// Resolve a supplier reference for Receiving. Suppliers must be active.
// Returns { supplierId, supplierName } or null when no supplierId is supplied.
async function resolveSupplier(supplierId, supplierText, tx = prisma) {
  if (supplierId === undefined || supplierId === null || supplierId === '') return null
  const supplier = await tx.supplier.findUnique({ where: { id: supplierId } })
  if (!supplier) throw new Error('Supplier not found')
  if (!supplier.isActive) throw new Error('Supplier is inactive and cannot be used in Receiving')
  return { supplierId: supplier.id, supplierName: supplier.name || supplierText }
}
...
export async function createReceivingDraft({ user, body }) {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const supplierRef = await resolveSupplier(body.supplierId, body.supplier, tx)
    ...
    const receiving = await tx.receiving.create({
      data: {
        ...
        supplier: supplierRef ? supplierRef.supplierName : (body.supplier || null),
        supplierId: supplierRef ? supplierRef.supplierId : null,
    ...
    await logAudit({
      ...
      after: { grnNumber, warehouseId: warehouse.id, stagingLocationId: staging.id, supplierId: receiving.supplierId, supplier: receiving.supplier, refDocument: receiving.refDocument, invoiceNumber: receiving.invoiceNumber, lineCount: lines.length },
    })
...
export async function updateReceivingDraft({ user, id, body }) {
  ...
  return await prisma.$transaction(async (tx) => {
      ...
      for (const l of body.lines) {
        await tx.receivingLine.create({
          data: {
            ...
            batchNo: l.batchNo || null,
            remarks: l.remarks || null,
            // Receiving Resolution (RCV-2.4) — variance disposition.
    ...
    // Resolve supplier reference (validate active) unless explicitly cleared.
    let supplierId = before.supplierId
    let supplier = before.supplier
    if (Object.prototype.hasOwnProperty.call(body, 'supplierId')) {
      if (body.supplierId === null || body.supplierId === '') {
        supplierId = null
        supplier = body.supplier !== undefined ? (body.supplier || null) : null
      } else {
        const supplierRef = await resolveSupplier(body.supplierId, body.supplier, tx)
      ...
      // Legacy free-text path — keep any existing supplierId reference intact.
    ...
    const updated = await tx.receiving.update({
      ...
      data: {
        supplier,
        supplierId,
        refDocument: body.refDocument !== undefined ? body.refDocument : before.refDocument,
        invoiceNumber: body.invoiceNumber !== undefined ? body.invoiceNumber : before.invoiceNumber,
        vehicleNumber: body.vehicleNumber !== undefined ? body.vehicleNumber : before.vehicleNumber,
        driverName: body.driverName !== undefined ? body.driverName : before.driverName,
    ...
    await logAudit({
      ...
      before: { supplierId: before.supplierId, supplier: before.supplier, refDocument: before.refDocument, invoiceNumber: before.invoiceNumber, lineCount: before.lines.length },
      after: { supplierId: updated.supplierId, supplier: updated.supplier, refDocument: updated.refDocument, invoiceNumber: updated.invoiceNumber, lineCount: updated.lines.length },
    })
    return updated
  })
}
...
// Receives part/all of the outstanding qty on the SAME GRN. Only updates the
// line's receivedQty / outstandingQty and records Outstanding Activity — no
// stock ledger, no FIFO, no putaway task, no new GRN.
export async function receiveOutstanding({ user, id, lines }) {
  const receiving = await prisma.receiving.findUnique({
    where: { id },
  ...
  const input = new Map((Array.isArray(lines) ? lines : []).map((l) => [l.lineId, Number(l.receivedQty) || 0]))

  // Pre-flight validation for every requested line.
  const requested = []
  for (const line of receiving.lines) {
    const qty = input.get(line.id)
    if (qty === undefined) continue
    const outstanding = Number(line.outstandingQty) || 0
    if (qty <= 0) throw new Error(`Line ${line.item.sku}: Receive Now must be greater than 0`)
    if (qty > outstanding) throw new Error(`Line ${line.item.sku}: Receive Now (${qty}) cannot exceed outstanding (${outstanding})`)
    requested.push({ line, qty, outstandingBefore: outstanding })
  }
  if (requested.length === 0) throw new Error('No outstanding lines to receive')

  const result = await prisma.$transaction(async (tx) => {
    for (const { line, qty, outstandingBefore } of requested) {
      const outstandingAfter = Math.max(0, outstandingBefore - qty)
      await tx.receivingLine.update({
        where: { id: line.id },
        data: {
          receivedQty: (Number(line.receivedQty) || 0) + qty,
          outstandingQty: outstandingAfter,
          // Auto complete the resolution when outstanding reaches zero.
          ...(outstandingAfter === 0 ? { resolution: 'Completed' } : {}),
        },
      })
      await tx.receivingOutstandingActivity.create({
        data: {
          receivingLineId: line.id,
          receivingId: receiving.id,
          itemId: line.itemId,
      ...
      await logAudit({
        ...
        entityId: line.id,
        description: `Outstanding receipt ${qty} for ${line.item.sku} on ${receiving.grnNumber} (outstanding ${outstandingBefore} -> ${outstandingAfter})`,
        before: { receivedQty: line.receivedQty, outstandingQty: outstandingBefore },
        after: { receivedQty: (Number(line.receivedQty) || 0) + qty, outstandingQty: outstandingAfter },
      })
    }
...
// Outstanding activity history for a Receiving document (same GRN).
export async function listOutstandingActivity(receivingId) {
  return prisma.receivingOutstandingActivity.findMany({
    where: { receivingId },
    include: { item: { select: { sku: true, name: true } }, operator: { select: { name: true } } },
    orderBy: { createdAt: 'desc' },
  })
...
export async function listReceivings({ status, warehouseId, take = 100 } = {}) {
  ...
  return prisma.receiving.findMany({
    ...
    include: {
      ...
      supplierRel: { select: { id: true, code: true, name: true, isActive: true } },
      lines: { select: { id: true, receivedQty: true, expectedQty: true, outstandingQty: true } },
    },
    orderBy: { createdAt: 'desc' },
