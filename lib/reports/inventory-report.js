...
function computeBucketSummary(rows, buckets, qtyKey = 'qty') {
  ...
  const lastLabel = `${buckets[buckets.length - 1].minDays}+`
  bucketTotals[lastLabel] = 0
...
export async function getInventoryReport(reportType, filters = {}) {
  ...
  switch (reportType) {
    case 'stock-on-hand':
      return queryStockOnHand(filters)
    case 'stock-card':
      return queryStockCard(filters)
    case 'inventory-aging':
      return queryInventoryAging(filters)
    case 'fifo-aging':
      return queryFifoAging(filters)
    case 'dead-stock':
      return queryDeadStock(filters)
    default:
