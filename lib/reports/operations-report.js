...
// Several models (StockTransfer, PutawayTask, PickingOrder) store user/location
// ids as plain scalars without relation fields, so names/codes are resolved here
// instead of via Prisma `include`.
async function resolveUserNames(records, idFields) {
  const ids = [...new Set(records.flatMap((r) => idFields.map((f) => r[f])).filter(Boolean))]
  if (!ids.length) return {}
  const users = await prisma.user.findMany({ where: { id: { in: ids } }, select: { id: true, name: true } })
  return Object.fromEntries(users.map((u) => [u.id, u.name]))
}

async function resolveItemMap(itemIds) {
...
async function resolveLocationMap(locationIds) {
  const ids = [...new Set(locationIds.filter(Boolean))]
  if (!ids.length) return {}
  const locs = await prisma.location.findMany({
    where: { id: { in: ids } },
    select: { id: true, code: true, type: true, zone: { select: { warehouse: { select: { code: true } } } } },
  })
...
async function queryReceiving({ warehouseId, fromDate, toDate, status, supplier, documentNumber, limit, offset }) {
  ...
  const [rows, total] = await Promise.all([
    prisma.receiving.findMany({
      ...
      include: {
        ...
        User_Receiving_createdByIdToUser: { select: { name: true } },
  ...
  const data = rows.map((r) => ({
    ...
    createdBy: r.User_Receiving_createdByIdToUser?.name || '—',
...
async function queryPutaway({ warehouseId, fromDate, toDate, status, itemId, documentNumber, limit, offset }) {
  ...
  const createdByName = await resolveUserNames(rows, ['createdById'])
  const completedByName = await resolveUserNames(rows, ['completedById'])
  ...
  const data = rows.map((t) => ({
    ...
    createdBy: createdByName[t.createdById] || '—',
    completedBy: completedByName[t.completedById] || '—',
...
async function queryMovement({ warehouseId, fromDate, toDate, status, itemId, documentNumber, limit, offset }) {
  ...
  const itemMap = await resolveItemMap(transfers.flatMap((t) => t.lines.map((l) => l.itemId)))
  const locMap = await resolveLocationMap(
    transfers.flatMap((t) => t.lines.flatMap((l) => [l.fromLocationId, l.toLocationId]))
  )
  const createdByName = await resolveUserNames(transfers, ['createdById'])
  ...
  for (const t of transfers) {
    for (const l of t.lines) {
      ...
      const fromLoc = locMap[l.fromLocationId]
      const toLoc = locMap[l.toLocationId]

      data.push({
        transferId: t.id,
        transferNumber: t.transferNumber,
        status: t.status,
        itemSku: itemMap[l.itemId]?.sku || '—',
        itemName: itemMap[l.itemId]?.name || '—',
        uom: itemMap[l.itemId]?.uom?.code || '—',
        qty: l.qty,
        fromLocation: fromLoc?.code || '—',
        fromWarehouse: fromLoc?.zone?.warehouse?.code || '—',
        toLocation: toLoc?.code || '—',
        toWarehouse: toLoc?.zone?.warehouse?.code || '—',
        createdAt: t.createdAt,
        postedAt: t.postedAt,
        createdBy: createdByName[t.createdById] || '—',
      })
...
async function queryPicking({ warehouseId, fromDate, toDate, status, assignedToId, documentNumber, limit, offset }) {
  ...
  const createdByName = await resolveUserNames(rows, ['createdById'])
  const assignedToName = await resolveUserNames(rows, ['assignedToId'])
  ...
  const data = rows.map((o) => {
    ...
    return {
      ...
      assignedTo: assignedToName[o.assignedToId] || '—',
      createdBy: createdByName[o.createdById] || '—',
