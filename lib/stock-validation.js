...
export async function validateLocation(codeOrId, { type = null, activeOnly = true, tx = prisma } = {}) {
  // Accept either a location id or a location code. Try id first, then code —
  // UUID ids contain hyphens but seeded/test ids may not, so a single heuristic
  // on the string shape is unreliable.
  let loc = await (tx || prisma).location.findUnique({ where: { id: codeOrId } })
  if (!loc) loc = await (tx || prisma).location.findUnique({ where: { code: codeOrId } })
...
export async function validateTransfer({ fromLocationId, toLocationId, lines }) {
  if (!Array.isArray(lines) || lines.length === 0) {
    // Legacy top-level shape: validate single from/to locations.
