...
export async function listPutawayTasks({ status, warehouseId, take = 100 } = {}) {
  ...
  return prisma.putawayTask.findMany({
    ...
    include: {
      ...
      receiving: { select: { id: true, grnNumber: true } },
    },
...
export async function completePutawayTask({ user, id, body }) {
  ...
  if (task.receiving?.warehouseId && destLoc.zone?.warehouseId !== task.receiving.warehouseId) {
  ...
  if (serialTracked) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 3. FIFO layer: create NEW layer at destination (never edit existing)
      //    The existing FIFO layer at staging remains as-is.
      //    New layer is created for the destination bin to support future FIFO picks.
      await tx.fifoLayer.create({
        data: {
          itemId: task.itemId,
          locationId: destLoc.id,
          refNumber: task.taskNumber,
          qtyReceived: qtyToPutaway,
          qtyRemaining: qtyToPutaway,
          unitCost: 0,
        },
      })
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // 3. FIFO layer: create NEW layer at destination
      await tx.fifoLayer.create({
        data: {
          itemId: task.itemId,
          locationId: destLoc.id,
