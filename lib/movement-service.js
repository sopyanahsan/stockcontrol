...
// Read-only helper: StockTransfer stores user ids as plain scalars (no relation
// fields), so names are resolved via a supplementary lookup instead of `include`.
...
export async function listMovements({ status, warehouseId, take = 100 } = {}) {
  ...
  const createdByName = await resolveUserNames(movements, ['createdById'])
  const approvedByName = await resolveUserNames(movements, ['approvedById'])

  const data = movements.map((m) => ({
    ...m,
    createdBy: m.createdById ? { id: m.createdById, name: createdByName[m.createdById] || null } : null,
    approvedBy: m.approvedById ? { id: m.approvedById, name: approvedByName[m.approvedById] || null } : null,
  }))
...
export async function getMovement(id) {
  ...
  const locIds = [...new Set(t.lines.flatMap((l) => [l.fromLocationId, l.toLocationId]))]

  const [items, locations, createdByName, approvedByName, reasonCode] = await Promise.all([
    prisma.item.findMany({
      where: { id: { in: itemIds } },
      select: { id: true, sku: true, name: true, uom: { select: { code: true } }, serialTracked: true },
    }),
    prisma.location.findMany({
      where: { id: { in: locIds } },
      select: { id: true, code: true, name: true },
    }),
    resolveUserNames([t], ['createdById']),
    resolveUserNames([t], ['approvedById']),
    t.reasonCodeId ? prisma.reasonCode.findUnique({ where: { id: t.reasonCodeId } }) : Promise.resolve(null),
  ])
  ...
  return {
    ...
    lines: t.lines.map((l) => ({
      ...l,
      item: itemMap[l.itemId] || null,
      fromLocation: locMap[l.fromLocationId] || null,
      toLocation: locMap[l.toLocationId] || null,
    })),
    createdBy: t.createdById ? { id: t.createdById, name: createdByName[t.createdById] || null } : null,
    approvedBy: t.approvedById ? { id: t.approvedById, name: approvedByName[t.approvedById] || null } : null,
    reasonCode,
  }
}
