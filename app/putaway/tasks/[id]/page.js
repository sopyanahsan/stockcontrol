...
const App = ({ params }) => {
  ...
  const cancelMut = useMutation({
    ...
    onSuccess: () => { toast.success('Task cancelled'); router.push('/putaway/tasks') },
    onError: (e) => toast.error(e.message),
  })
    ...
    return (
      <AppShell title="Putaway Task" subtitle="Not found">
        <div className="rounded-md border p-8 text-center text-sm text-gray-500">
          Task not found. <Link className="text-blue-600 underline" href="/putaway/tasks">Back to queue</Link>
        </div>
      </AppShell>
    )
  }
    ...
    <AppShell
      ...
      actions={
        <Button asChild variant="outline" size="sm" className="h-8">
          <Link href="/putaway/tasks"><ArrowLeft className="mr-1 h-4 w-4" /> Back</Link>
        </Button>
      }
    >
              ...
              <div className="flex gap-2">
                ...
                {isCompleted && (
                  <Button asChild size="sm" variant="outline" className="h-8">
                    <Link href="/putaway/tasks"><PackageCheck className="mr-1 h-3.5 w-3.5" /> Back to Queue</Link>
                  </Button>
                )}
              </div>
            </div>
          ...
          <div className="rounded-md border border-green-200 bg-green-50 p-6 text-center">
            ...
            <div className="mt-3 flex justify-center gap-2">
              <Button asChild size="sm" variant="outline">
                <Link href="/putaway/tasks"><ArrowLeft className="mr-1 h-3.5 w-3.5" /> Back to Queue</Link>
              </Button>
            </div>
          </div>
        )}
