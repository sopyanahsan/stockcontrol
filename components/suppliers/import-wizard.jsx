...
async function downloadErrorReport(errors, fileName) {
  const XLSX = await import('xlsx')
  const rows = errors.map((e) => [e.row, e.code || '', (e.reasons || []).join('; ')])
  const ws = XLSX.utils.aoa_to_sheet([['Row', 'Code', 'Reason'], ...rows])
  ws['!cols'] = [{ wch: 8 }, { wch: 20 }, { wch: 55 }]
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'Errors')
  XLSX.writeFile(wb, `${String(fileName).replace(/\.xlsx$/i, '') || 'suppliers'}-errors.xlsx`)
}
...
export default function SupplierImportWizard({ open, onOpenChange, onSuccess }) {
  ...
  const runValidation = async () => {
    ...
    try {
      const res = await fetch('/api/suppliers/import', { method: 'POST', body: form, credentials: 'include' })
  ...
  const runImport = async () => {
    ...
    try {
      ...
      setStep(4)
      onSuccess && onSuccess()
      toast.success(`Imported ${finalResult?.imported || 0} supplier(s)`)
    } catch (err) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-base">Import Suppliers (Excel)</DialogTitle>
                  ...
                  <tbody>
                    {summary.errors.slice(0, 100).map((e, i) => (
                      <tr key={i} className="border-b border-red-100">
                        ...
                        <td className="px-3 py-1.5 font-mono text-gray-700">{e.code || '-'}</td>
            ...
            {!result ? (
              <div className="space-y-2">
                <div className="flex justify-between text-xs text-gray-500">
                  <span className="flex items-center gap-1.5"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Importing suppliers...</span>
